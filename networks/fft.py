...
class FourierLayer(nn.Module):
    ...
    def __init__(self, pred_len, k=None, low_freq=0, output_attention=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return self.extrapolate(x_freq, f, t), x_freq, f
    ...
    def topk_freq(self, x_freq):
        topk = min(self.k, x_freq.size(0))
        values, indices = torch.topk(x_freq.abs(), topk, dim=1, largest=True, sorted=True)
    ...
    def dft_forward(self, x):
        ...
        topk = min(self.k, x_freq.size(0))
        _, indices = torch.topk(x_freq.abs(), topk, dim=1, largest=True, sorted=True)
